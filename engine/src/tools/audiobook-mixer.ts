...
export async function mixAudiobook(
  params: MixAudiobookParams
): Promise<MixAudiobookResult> {
  ...
  try {
    ...
    let musicBedPath: string | undefined;
    if (hasMusic) {
    ...
    return {
      success: false,
      error: `Mix failed: ${(err as Error).message}`,
    };
  }
}
...
async function buildMusicBed(
  ...
  for (let si = 0; si < manifest.scenes.length; si++) {
    if (si > 0) cursor += SCENE_BREAK_MS;
    sceneStarts.push(cursor);

    const scene = manifest.scenes[si];
    for (let segi = 0; segi < scene.segments.length; segi++) {
      if (segi > 0) cursor += SEGMENT_GAP_MS;
  ...
  const sceneMusicPaths: Array<{ path: string; startMs: number; durationMs: number }> = [];

  for (let si = 0; si < manifest.scenes.length; si++) {
    const scene = manifest.scenes[si];
    if (!scene.music_file_path) continue;
  ...
  if (sceneMusicPaths.length === 0) return;
  ...
  if (sceneMusicPaths.length === 1) {
    ...
    const lines: string[] = [];
    if (p.startMs > 0) lines.push(`file '${silenceBefore}'`);
    lines.push(`file '${p.path}'`);
    if (endMs < totalDurationMs) lines.push(`file '${silenceAfter}'`);
 ...
 */
function collectSfxTimestamps(manifest: AudioManifest): SfxEntry[] {
...
async function finalMix(
  ...
  const inputs: string[] = ["-i", spinePath];
  let inputIndex = 1;
  const filterParts: string[] = [];
