...
export function parseAndRoll(expression: string): RollResult {
...
export interface DiceToolResult {
  ...
  modifier: number;
  total: number;
  advantage?: { rolls: [number[], number[]]; chosen: "first" | "second" };
  purpose?: string;
}

export function rollDice(params: DiceToolParams): DiceToolResult {
