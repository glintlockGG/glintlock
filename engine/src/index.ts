...
server.tool(
  "roll_oracle",
  "Roll on a random oracle table from the game rules. Returns a real random result from curated content. Use instead of inventing NPCs, encounters, treasure, etc.",
  {
    table: z.enum([
...
server.tool(
  "oracle_yes_no",
  "Roll a yes/no oracle with set odds (Ironsworn-style). Use for ambiguous world-state questions, NPC decisions, doom advancement checks, and any uncertain outcome that needs a fair random answer.",
  {
...
server.tool(
  "generate_sfx",
  "Generate and play a sound effect. Use for environmental sounds (door creaks, dripping water), combat impacts (sword clashes, spell effects), monster roars, treasure clinking, and atmospheric ambience. Audio plays in background — does not block.",
  {
...
server.tool(
  "play_music",
  "Control background music. Use to set mood when entering new areas or shifting tone. Music loops automatically and layers with TTS/SFX through Core Audio. Only one music track plays at a time — playing new music stops the previous track.",
  {
...
server.tool(
  "list_voices",
  "Browse available ElevenLabs voices. Use when creating important NPCs to find a fitting voice. Store the chosen voice_id in the NPC's frontmatter for consistent dialogue.",
  {
...
server.tool(
  "render_audiobook",
  "Parse a chronicle chapter markdown into a structured audio manifest (segments, chunks, voice assignments, estimated durations). Pure text processing — no API calls. Use before generating TTS/SFX/music for an audiobook.",
  {
...
server.tool(
  "mix_audiobook",
  "Mix rendered audiobook segments into a final MP3 using ffmpeg. Concatenates speech with gaps, layers music bed and SFX, applies fade-in/fade-out. Requires ffmpeg installed.",
  {
