...
// --- get_entity ---
server.tool("get_entity", "Read an entity and all its components from the world state. Query by ID, name, or type. Returns the entity with all attached component data.", {
...
// --- create_entity ---
server.tool("create_entity", `Create a new entity with initial components. The entity ID is auto-generated from type and name (e.g. 'npc_merchant_vela').
...
// --- query_entities ---
server.tool("query_entities", "Query entities by type, location, or component values. Returns matching entities with all their components.", {
    entity_type: z.enum(["pc", "npc", "location", "item", "faction"]).optional().describe("Filter by entity type"),
...
// --- delete_entity ---
server.tool("delete_entity", "Delete an entity and all its components permanently. CASCADE deletes component rows and attached notes. Use for: dead NPCs, consumed items, test cleanup. Requires confirm: true.", {
...
// --- roll_oracle ---
server.tool("roll_oracle", "Roll on a random oracle table from the Shadowdark RPG system. Returns a real random result from curated content. Use instead of inventing NPCs, encounters, treasure, etc.", {
    table: z.enum([
...
// --- add_note ---
server.tool("add_note", "Record a freeform text note. Can be global (campaign-level) or attached to a specific entity. Use for: session events, NPC promises, unresolved plot threads, player decisions, rulings made. Notes survive context compaction.", {
...
// --- query_notes ---
server.tool("query_notes", "Search and filter session notes. Query by entity, tag, text content, or time range. Returns notes in reverse chronological order.", {
...
// --- get_session_summary ---
server.tool("get_session_summary", "Get a comprehensive summary of the current campaign state for cold starts and session recaps. Returns PC status, current location, recent events, active threads, and key NPCs.", {
